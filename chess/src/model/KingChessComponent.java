...
public class KingChessComponent extends ChessComponent{
     ...
     */
        ...
        public KingChessComponent(ChessboardPoint chessboardPoint, Point location, ChessColor color, ClickController listener, int size,String name) {
            ...
            if (color == ChessColor.WHITE){name = "k";}
            if (color == ChessColor.BLACK){name = "K";}
        }
    public static ChessboardPoint getKing(ChessComponent[][]chessComponents,ChessColor color) {
        ...
        public boolean canMoveTo(ChessComponent[][] chessComponents, ChessboardPoint destination) {
            ...
            ChessboardPoint toMove = new ChessboardPoint(source.getX(), source.getY() + 1);


            if (toMove.getX() < 8 && toMove.getX() > -1 && toMove.getY() < 8 && toMove.getY() > -1) {
                movedChess = chessComponents[source.getX()][source.getY() + 1];
                if (movedChess.getChessColor() != sourceChess.getChessColor()) {
                    canMoveList.add(toMove);
                }
            }

            toMove = new ChessboardPoint(source.getX(), source.getY() - 1);
            ...
            toMove = new ChessboardPoint(source.getX() + 1,source.getY());

            if (toMove.getX() < 8 && toMove.getX() > -1 && toMove.getY() < 8 && toMove.getY() > -1) {
                movedChess = chessComponents[source.getX()+1][source.getY() ];
                if (movedChess.getChessColor() != sourceChess.getChessColor()) {
                    canMoveList.add(toMove);
                }
            }
            ...
            toMove = new ChessboardPoint(source.getX() + 1, source.getY() + 1);

            if (toMove.getX() < 8 && toMove.getX() > -1 && toMove.getY() < 8 && toMove.getY() > -1) {
                movedChess = chessComponents[source.getX()+1][source.getY() + 1];
                if (movedChess.getChessColor() != sourceChess.getChessColor()) {
                    canMoveList.add(toMove);
                }
            }
